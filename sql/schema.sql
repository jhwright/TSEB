-- Canonical TSEB schema (reflects post-migration state). For history, see 001_schema.sql + 003_consolidate_status.sql
...
create table institutions (
  ...
  status text not null default 'initial_contact' check (status in (
    'initial_contact','in_conversation','site_visit','active','on_hold','previous','inactive'
  )),
  inactive_reason text,       -- why inactive, if applicable
  next_step text,
  next_step_due date,
